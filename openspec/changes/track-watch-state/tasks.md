...
- [ ] 2.1 `it_should_record_playback_progress`: drives saving the position of an unwatched video (`VideoWatchStateUpdater::record_progress`, `find_by_youtube_id`, `Video::record_progress`).
...
- [ ] 2.6 `it_should_mark_a_watched_video_unwatched_past_10_percent_of_a_rewatch`: drives the rewatch reset.
...
- [ ] 2.14 `it_should_count_unwatched_downloaded_videos_when_listing_channels`: drives `ChannelSearcher` computing `unwatched_count` and the flat channel list mapping.
- [ ] 2.15 `it_should_include_watch_state_when_listing_channel_videos`: drives the `watched`/`position_seconds` mapping on `VideoResponse`.
- [ ] 2.16 `it_should_include_watch_state_when_listing_playlist_videos`: drives the same fields through the playlist listing.
