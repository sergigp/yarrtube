...
The system SHALL provide an HTTP endpoint that creates a playlist given a YouTube playlist ID, a name, a storage path, and a quality tier (`high`, `mid`, or `low`), confirms the YouTube playlist ID corresponds to an existing, accessible YouTube playlist before persisting, and stores the playlist with that path, quality, and a system-generated creation timestamp.
...
- **THEN** the system persists a playlist record with that ID, name, path, quality, and a creation timestamp, and returns the created playlist
...
- **THEN** the system returns all of them, each with its ID, name, path, quality, and creation timestamp
