...
- **WHEN** a client requests the list of non-completed tasks and a task's payload references a specific video (for example, a download or file-cleanup task)
- **THEN** the returned task includes that video's title and ID in addition to the playlist name

#### Scenario: Referenced playlist or video no longer exists
- **WHEN** a client requests the list of non-completed tasks and a task's payload references a playlist or video that can no longer be found
