...
- [x] 1.1 Add a `ScheduledTask` entity (id, task_type, payload, status
      ...
      to Running.
- [x] 1.2 Add `ScheduledTask::fail(error, now) -> TaskFailureOutcome`
...
- [x] 2.1 Add a `ScheduledEvent` entity (id, event_type, payload, retries,
...
- [x] 3.1 Change `TaskRepository` trait to `schedule` (create), `find(id)`,
      ...
      `recover_running`.
- [x] 3.2 Reimplement `SqliteTaskRepository` against the new trait, dropping
...
- [x] 4.1 Change `EventRepository` trait to `insert_pending` (create),
      `find(id)`, `list_eligible()`, `update(&ScheduledEvent)`, `delete(id)`,
      `dead_letter(&DeadLetteredEvent)`, removing `mark_done`/
      `mark_failed_or_retry`.
- [x] 4.2 Reimplement `SqliteEventRepository` against the new trait, and
...
- [x] 5.1 Add a `Clock` dependency to `TaskExecutor`, rewrite `poll_once` to
...
- [x] 5.2 Replace `recover_running()` with a startup routine (in
...
- [x] 5.3 Add a `Clock` dependency to `DomainEventsConsumer`, rewrite
...
- [x] 6.1 Update `serve.rs::build_application` to construct
...
- [x] 7.1 Update `.claude/skills/rust-architect/SKILL.md`'s CQS section to
