...
The system SHALL provide a command that accepts exactly two positional arguments: a YouTube playlist URL and a local output directory path.

#### Scenario: Valid invocation
- **WHEN** the user runs the command with a playlist URL and an output path
...
The system SHALL resolve a playlist URL into an ordered list of member videos (URL and title) by querying the YouTube Data API v3 `playlistItems` resource, following pagination until all pages are retrieved.
...
- **WHEN** the playlist URL does not correspond to an existing, accessible playlist, or the API returns an error
