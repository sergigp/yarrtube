...
mod tests {
    ...
    fn video_reconciler(
    ...
    ) -> (VideoReconciler, Arc<FakeTaskRepository>) {
        ...
        (video_reconciler, task_repository)
    }

    #[test]
    fn it_should_no_op_when_the_payload_playlist_id_is_invalid() {
        let (video_reconciler, task_repository) =
            video_reconciler(Arc::new(FakePlaylistRepository::default()));
